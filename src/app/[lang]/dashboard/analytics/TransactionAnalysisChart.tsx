...
const TransactionAnalysisChart: React.FC<TransactionAnalysisProps> = ({
  data = [],
  timeframe = "weekly",
  period = "current",
  dateRange = {
  ...
  const paymentMethods = Array.from(
    new Set(data.flatMap((item) => Object.keys(item.paymentBreakdown)))
  ).sort((a, b) => {
    const totalA = data.reduce(
      (sum, item) => sum + (item.paymentBreakdown[a]?.totalAmount || 0),
      0
    );
    const totalB = data.reduce(
      (sum, item) => sum + (item.paymentBreakdown[b]?.totalAmount || 0),
      0
    );
  ...
  const brandColors = {
    ...
    secondary: {
      base: "rgba(245, 140, 123, %a)",
      solid: "rgba(245, 140, 123, 1)",
    },
  ...
  const chartData: ChartData<"bar"> = {
    ...
    datasets: paymentMethods.map((method, index) => {
      const colorKey = ["primary", "secondary", "accent", "gray", "darkGray"][
        index % 5
      ] as keyof typeof brandColors;
      return {
        label: method.toUpperCase(),
        data: data.map(
          (item) => item.paymentBreakdown[method]?.totalAmount || 0
        ),
