      ...
      <div className="flex overflow-hidden flex-col px-8 py-6 w-full bg-neutral-900 rounded-[18px] max-md:max-w-full text-white min-h-[calc(100vh-73px)]">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder={translations.transactions?.search?.placeholder || "Search transactions..."}
              value={searchQuery}
        ...
        <div className="mb-6">
          <h2 className="text-lg font-medium text-white mb-4">
            {translations.transactions?.title || "Transactions"}
          </h2>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="text-white">
                  <tr className="text-start">
                    <th className="p-2">{translations.transactions?.table?.id || "ID"}</th>
                    <th className="p-2">{translations.transactions?.table?.from || "From"}</th>
                    <th className="p-2">{translations.transactions?.table?.provider || "Provider"}</th>
                    <th className="p-2">{translations.transactions?.table?.amount || "Amount"}</th>
                    <th className="p-2">{translations.transactions?.table?.state || "Status"}</th>
                    <th className="p-2">{translations.transactions?.table?.userName || "User Name"}</th>
                    <th className="p-2">{translations.transactions?.table?.date || "Date"}</th>
                    <th className="p-2">{translations.transactions?.table?.action || "Action"}</th>
                  </tr>
                </thead>
                <tbody>
                  {displayedTransactions.length > 0 ? (
                      ...
                      >
                        ...
                        <td className="p-2 text-xs">
                          {transaction.state === "pending" ? (
                            <span className="text-[#F58C7B] bg-[#F58C7B] bg-opacity-20 px-2 py-[2px] rounded-full">
                              {translations.transactions?.status?.pending || "Pending"}
                            </span>
                          ) : (
                            <span className="text-[#53B4AB] bg-[#53B4AB] bg-opacity-20 px-2 py-[2px] rounded-full">
                              {translations.transactions?.status?.completed || "Completed"}
                            </span>
                          )}
                        </td>
                        ...
                        <td className="p-2">
                          <div className="flex gap-2 items-center">
                            ...
                            {transaction.state === "pending" && (
                              <span
                                className="text-[#c25443] bg-[#F58C7B] bg-opacity-20 cursor-pointer px-2 py-[2px] rounded-full text-xs hover:bg-opacity-30 transition-colors"
                                onClick={(e) => {
                                  e.stopPropagation();
                              ...
                              >
                                {translations.transactions?.action?.markAsCompleted || "Mark as Completed"}
                              </span>
                            )}
                          </div>
                        </td>
                    ...
                    <tr>
                      <td className="p-4 text-center text-gray-400" colSpan={8}>
                        {translations.transactions?.noData || "No transactions found"}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="fixed w-full z-20 inset-0 bg-black bg-opacity-70 flex justify-center items-center">
            <div className="bg-neutral-800 p-4 rounded-lg text-white">
              <h2 className="text-lg font-semibold">
                {translations.transactions?.confirmModal?.title || "Confirm Action"}
              </h2>
              <p className="mt-2">
                {translations.transactions?.confirmModal?.message || "Are you sure you want to mark this transaction as completed?"}
              </p>
              <div className="mt-4 flex justify-end space-x-2 gap-2">
                ...
                >
                  {translations.transactions?.confirmModal?.cancel || "Cancel"}
                </button>
                <button
                  className="px-4 py-2 bg-[#53B4AB] text-black rounded-lg text-xs"
                  onClick={handleMarkAsCompleted}
                >
                  {translations.transactions?.confirmModal?.confirm || "Confirm"}
                </button>
              </div>
            </div>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <span className={`px-2 py-[2px] rounded-full text-xs ${
